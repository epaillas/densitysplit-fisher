...
def likelihood_full(nderiv=1500, ncov=7000, smin=0, smax=150):
    smin = 10
    smax = 150
    ells = (0, 2)
    ...
    # samples to plot
    samples_plot = ['Om', 'Ob2', 'Mnu', 'h', 's8', 'ns']
    ...
    legend = [
        ...
        r'${\rm DS_{5}^{\rm qq + qh}}$',
        r'${\rm DS_{1}^{\rm qq + qh}}$',
        # r'${\rm DS_{1+2+3+4+5}^{\rm qq + qh}}$',
        r'${\rm DS_{1+2+4+5}^{\rm qq + qh}}$',
    ]

    # calculate fisher matrices
    tpcf = tpcf_fisher_matrix(samples=samples, ncov=ncov, nderiv=nderiv,
        smin=smin, smax=smax, ells=ells, covariance_correction=covariance_correction)

    ds5 = ds_fisher_matrix(samples=samples, ncov=ncov, nderiv=nderiv,
    ...
    ds1 = ds_fisher_matrix(samples=samples, ncov=ncov, nderiv=nderiv,
        smin=smin, smax=smax, split='z', ells=ells, quantiles=[1],
        correlation_type=correlation_type, covariance_correction=covariance_correction)

    # ds_12345 = ds_fisher_matrix(samples=samples, ncov=ncov, nderiv=nderiv,
    #     smin=smin, smax=smax, split='z', ells=ells, quantiles=[1, 2, 3, 4, 5],
    #     correlation_type=correlation_type, covariance_correction=covariance_correction)
    ...
    fisher_list.append(ds5)
    fisher_list.append(ds1)
    # fisher_list.append(ds_12345)
    ...
    plt.annotate(r'$s_{\mathrm{min}} =\ $' + r'${}$'.format(smin) +\
        r'$\,h^{-1}{\rm Mpc}$', xy=(0.76, 0.75), xycoords='figure fraction',
        bbox=dict(ec='0.5', fc='w', boxstyle='round'), fontsize=26)

    plt.savefig('paper_figures/pdf/likelihood_full.pdf')
    plt.savefig('paper_figures/png/likelihood_full.png', dpi=300)
