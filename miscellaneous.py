...
dtype2ctype = {np.dtype(np.float64): 'double', np.dtype(np.float32): 'float', np.dtype(np.int32): 'int', np.dtype(np.uint64): 'size_t', np.dtype(np.int16): 'short int'}


def throwError(s1='',s2='',s3='',s4='',s5='',s6='',s7='',s8='',s9='',s10='',EXIT=True):
...
def checkArraySizes(shapeArray,nameArray,funcName):
    ...
    if len(shapeArray)!=len(nameArray):
        print("Could not check that the input arrays to function '%s' have the same dimensions!".format(funcName))
    for i in range(1,len(shapeArray)):
        if shapeArray[0]!=shapeArray[i]:
            throwError( "The '%s' and '%s' arguments of function '%s' must be numpy arrays having the same shape. The '%s' shape = %s while the '%s' shape = %s." % (nameArray[0],nameArray[i],funcName,nameArray[0],str(shapeArray[0]),nameArray[i],str(shapeArray[0])) )
...
def readArrayEntry(array,file,startPosition,noEntries):
    ...
    for i in range(noChuncks):
        array[startPosition:startPosition+maxChunckOfData] = np.fromfile( file, array.dtype, maxChunckOfData )
        startPosition += maxChunckOfData
    noLeftOver = int(noEntries - maxChunckOfData * noChuncks)
    array[startPosition:startPosition+noLeftOver] = np.fromfile( file, array.dtype, noLeftOver )
    return array
...
def return_raDecDist_coordinates( pos, angleUnit='degree' ):
    ...
    theta = np.pi/2. - np.arccos( pos[:,2]/dis )            # the declination
    sinTheta = np.sin( np.arccos( pos[:,2]/dis ) )
    phi   = np.pi + np.arctan2( pos[:,1]/(dis*sinTheta), pos[:,0]/(dis*sinTheta) )      # the right ascenssion
    
    if angleUnit in ['degree']:
        theta *= 180./np.pi
        phi   *= 180./np.pi
    elif angleUnit not in ['radian']:
        throwError( "Unknown angle unit in the function 'return_raDecDist_coordinates'. " )
