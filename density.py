...
densityMethod = {1:'DTFE', 2:'TSC', 3:'SPH', 11:'CIC', np.uint64(-1):'Unknown', -1:'Unknown' }
densityFileType = { DENSITY_FILE:"Density", VELOCITY_FILE:"Velocity", VELOCITY_GRADIENT_FILE:"Velocity gradient", VELOCITY_DIVERGENCE_FILE:"Velocity divergence", VELOCITY_SHEAR_FILE:"Velocity shear", VELOCITY_VORTICITY_FILE:"Velocity vorticity", VELOCITY_STD_FILE:"Velocity standard deviation", SCALAR_FIELD_FILE:"Scalar field", SCALAR_FIELD_GRADIENT_FILE:"Scalar field gradient", GRAVITATIONAL_POTENTIAL_FILE:"gravitational potential", WATERSHED_FILE:"Watershed Void Finder void index", -1:"Unknown" }
densityVariableName = { DENSITY_FILE:"density", VELOCITY_FILE:"velocity", VELOCITY_GRADIENT_FILE:"velocityGradient", VELOCITY_DIVERGENCE_FILE:"velocityDivergence", VELOCITY_SHEAR_FILE:"velocityShear", VELOCITY_VORTICITY_FILE:"velocityVorticity", VELOCITY_STD_FILE:"velocityStd", SCALAR_FIELD_FILE:"scalarField", SCALAR_FIELD_GRADIENT_FILE:"scalarFieldGradient", GRAVITATIONAL_POTENTIAL_FILE:"gravitationalPotential", WATERSHED_FILE:'voidIndex', -1:"unknown",   10001:'oneComponent', 10002:'twoComponent', 10003:'threeComponent'}
densityDataType = { DENSITY_FILE:'f4', VELOCITY_FILE:'f4', VELOCITY_GRADIENT_FILE:'f4', VELOCITY_DIVERGENCE_FILE:'f4', VELOCITY_SHEAR_FILE:'f4', VELOCITY_VORTICITY_FILE:'f4', VELOCITY_STD_FILE:'f4', SCALAR_FIELD_FILE:'f4', SCALAR_FIELD_GRADIENT_FILE:'f4', GRAVITATIONAL_POTENTIAL_FILE:'f4', WATERSHED_FILE:'i4', -1:'f4',  10001:'f4', 10002:'f4', 10003:'f4'}
densityDataComponents = { DENSITY_FILE:1, VELOCITY_FILE:3, VELOCITY_GRADIENT_FILE:9, VELOCITY_DIVERGENCE_FILE:1, VELOCITY_SHEAR_FILE:5, VELOCITY_VORTICITY_FILE:3, VELOCITY_STD_FILE:1, SCALAR_FIELD_FILE:NO_SCALAR_COMPONENTS, SCALAR_FIELD_GRADIENT_FILE:3*NO_SCALAR_COMPONENTS,GRAVITATIONAL_POTENTIAL_FILE:1, WATERSHED_FILE:1, -1:1,   10001:1, 10002:2, 10003:3 }
...
class DensityHeader:
    ...
    def nbytes(self):
        __size =  self.gridSize.nbytes + self.totalGrid.nbytes + self.fileType.nbytes + self.noDensityFiles.nbytes + self.densityFileGrid.nbytes + self.indexDensityFile.nbytes + self.box.nbytes + self.npartTotal.nbytes + self.mass.nbytes + self.time.nbytes + self.redshift.nbytes + self.BoxSize.nbytes + self.Omega0.nbytes + self.OmegaLambda.nbytes + self.HubbleParam.nbytes + self.method.nbytes + self.fill.nbytes + self.FILE_ID.nbytes
        return __size
    
    def dtype(self):
        __dt = np.dtype([ ('gridSize',np.uint64,3), ('totalGrid',np.uint64), ('fileType',np.int32), ('noDensityFiles',np.uint32), ('densityFileGrid',np.uint32,3), ('indexDensityFile',np.uint32), ('box',np.float64,6), ('npartTotal',np.uint64,6), ('mass',np.float64,6), ('time',np.float64), ('redshift',np.float64), ('BoxSize',np.float64), ('Omega0',np.float64), ('OmegaLambda',np.float64), ('HubbleParam',np.float64), ('method',np.uint64), ('fill','c',DensityHeader.fillSize), ('FILE_ID',np.int64) ])
        return __dt
    ...
    def fromfile(self,f, BUFFER=True, numBytes_data=None):
        ...
        if BUFFER:
            ...
            if ( __buffer1!=__buffer2 or __buffer1!=DensityHeader.byteSize ):
                throwError( "Error reading the header of the density file. 'buffer1'=", __buffer1, "and 'buffer2'=", __buffer2, "when both should be", DensityHeader.byteSize )
    ...
    def tofile(self,f,BUFFER=True):
        ...
        if self.nbytes()!=DensityHeader.byteSize:
            throwError("When calling the function 'DensityHeader.tofile()'. The size of the density header is %i while the expected size is %i. Please check which variables do not have the correct size." % (self.nbytes(),DensityHeader.byteSize))
    ...
    def FileType(self):
        __fileType = 'Unknown'
        if self.fileType in densityFileType: __fileType = densityFileType[ self.fileType ]
        return ( self.fileType, __fileType )
    
    def DataType(self):
        """ Returns the type of the data in the file, NOT the type of the header. """
        __dataType = 'f4'
        if self.fileType in densityDataType: __dataType = densityDataType[ self.fileType ]
        return __dataType
    
    def DataComponents(self):
        """ Returns the number of components of the data in the file. """
        __noComponents = 1
        if self.fileType in densityDataComponents: __noComponents = densityDataComponents[ self.fileType ]
        return __noComponents
    
    def DataName(self):
        """ Returns the name of the variable stored in the file. """
        __dataName = "unknown"
        if self.fileType in densityVariableName: __dataName = densityVariableName[ self.fileType ]
        return __dataName
    
    def Method(self):
        __method = 'Unknown'
        if self.method in densityMethod: __method = densityMethod[ self.method ]
...
def readDensityHeader(file,VERBOSE=True):
    ...
    if not os.path.isfile(tempName):
        ...
        if not os.path.isfile(tempName):  throwError( "Cannot find the density binary file. There are no '%s' or '%s' files." %(file,tempName) )
    if VERBOSE: print("Reading the header of the density file '{}' ... ".format(tempName))
    f = open(tempName, 'rb')
    header.fromfile( f )
...
def readDensityData( file, HEADER=True, VERBOSE=True, wrongFileLabel=False, dataType=None, dataComponents=None ):
    ...
    if header.noDensityFiles>1:
        for i in range(header.noDensityFiles):
            ...
            if not os.path.isfile(tempName):  throwError( "Cannot find the density binary file number %i (of %i files) with expected name '%s'." %(i+1,header.noDensityFiles,tempName) )
    ...
    if data.nbytes != numDataBytes  and  header.noDensityFiles==1:
        throwWarning( "There is a problem with the input file. The length in bytes of the data does not match the value expected from the file header!" )
        if wrongFileLabel:
            dataComponents = np.int( numDataBytes * 1. / header.totalGrid / np.array( [1], dataType).nbytes )
            print(dataComponents)
            if numDataBytes != dataComponents * header.totalGrid * np.array( [1], dataType).nbytes:
                throwError( "Cannot estimate what is the data format and size of the data array. Please supply these values as input to the read in function via the parameters 'dataType'." )
        else:
            throwError( "Program stopped since does not know how to read the input file!" )
    startPosition = 0
    
    for i in range(header.noDensityFiles):
        if VERBOSE: print("Reading the data in the density file '{}' which is file {} of {} files ... ".format(tempName,i+1,header.noDensityFiles))
        tempName = file
        if header.noDensityFiles!=1: tempName = densityMultipleFiles( file, i )
        if not os.path.isfile(tempName):  throwError( "Cannot find the density file number %i with expected name '%s'." %(i+1,tempName) )
        f = open(tempName, 'rb')
        ...
        __buffer2 = np.fromfile( f, bufferType, 1 )[0]
        if __buffer1!=__buffer2: throwError( "While reading the density data block in file '%s'." % tempName, "The buffers before and after the data do not have the same value (buffer1 = %i, buffer2 = %i while the expected value was %i)." %(__buffer1,__buffer2,dataSize*data[0].nbytes) )
...
def writeDensityData(file,header,data,VERBOSE=True):
    ...
    if VERBOSE: print(f'Writing the density data to the file {file}')
    __temp = header.gridSize[0] * header.gridSize[1] * header.gridSize[2]
    if __temp!=header.totalGrid:
        throwError( "The total number of grid points in the density header is not equal to the product of the grid dimensions along each axis. Total number of grid points is %i while the size along each axis is:" % header.totalGrid, header.gridSize )
    dataComponents = header.DataComponents()
    noDataElements = data.size/dataComponents
    if header.totalGrid!=noDataElements:
        throwError( "The total number of grid points in the density header does not agree with the data length. Number grid points in the header is %i while the data has only %i elements." % (header.totalGrid,noDataElements))
