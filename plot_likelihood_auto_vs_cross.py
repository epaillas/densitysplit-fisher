...
def likelihood_auto_vs_cross(nderiv=1500, ncov=7000, smin=0, smax=150):
    smin = 10
    smax = 150
    ells = (0, 2)
    quantiles = [5, 1]
    ...
    legend = [r'${\rm DS_{1+2+3+4+5}^{\rm qq}}$'] \
        + [r'${\rm DS_{1+2+3+4+5}^{\rm qh}}$'] \
        + [r'${\rm DS_{1+2+3+4+5}^{\rm qq+qh}}$']

    # calculate fisher matrices
    ds_auto = ds_fisher_matrix(samples=samples, ncov=ncov,
        nderiv=nderiv, smin=smin, smax=smax, split='z', ells=ells,
        quantiles=[1, 2, 3, 4, 5], correlation_type='auto')

    ds_cross = ds_fisher_matrix(samples=samples, ncov=ncov,
        nderiv=nderiv, smin=smin, smax=smax, split='z', ells=ells,
        quantiles=[1, 2, 3, 4, 5], correlation_type='cross')

    ds_full = ds_fisher_matrix(samples=samples, ncov=ncov,
        nderiv=nderiv, smin=smin, smax=smax, split='z', ells=ells,
        quantiles=[1, 2, 3, 4, 5], correlation_type='full')

    fisher_list.append(ds_auto)
    ...
    plt.savefig('support_figures/pdf/likelihood_auto_vs_cross.pdf')
    plt.savefig('support_figures/png/likelihood_auto_vs_cross.png', dpi=300)

if __name__ == '__main__':

    likelihood_auto_vs_cross()
